...
| `TASK-0009/T09-Y1-y90-s3401/summary.json` | `TASK-0009` | yaw `90` transfer-batch winner using the promoted non-energy route directly under `paper_ild_push_v1`; retained strict four-down improvement against the current yaw `90` baseline | retained-yaw90-transfer |
| `TASK-0009/T09-Y1-y90-s3401/comparison_summary.json` | `TASK-0009` | Retained-checkpoint comparison for the exact yaw `90` transfer; keeps all four metrics below the current yaw `90` baseline and wins the transfer batch on retained composite | retained-yaw90-transfer |
| `TASK-0009/T09-Y2-y90-s3401/summary.json` | `TASK-0009` | yaw `90` companion transfer with light late `dmag` protection under `paper_ild_push_v1_late_dmag_guard` | retained-yaw90-transfer |
| `TASK-0009/T09-Y2-y90-s3401/comparison_summary.json` | `TASK-0009` | Retained-checkpoint comparison for the yaw `90` late-`dmag` guard variant; also satisfies strict four-down improvement, but ranks behind `T09-Y1-y90-s3401` on retained composite | retained-yaw90-transfer |
| `TASK-0009/T09-R1-y90-s3401/summary.json` | `TASK-0009` | Promoted yaw `90` long run from the transfer-batch winner under `paper_ild_push_v1`; retained composite `3.004526558601078` at best iteration `7912` | retained-yaw90-long-run |
| `TASK-0009/T09-R1-y90-s3401/comparison_summary.json` | `TASK-0009` | Promoted yaw `90` long-run retained comparison; strongest yaw `90` composite / ILD / ITD / NMSE result so far, but it gives back normalized magnitude relative to the yaw `90` baseline | retained-yaw90-long-run |
| `TASK-0009/yaw90_transfer_followup_SESSION-P2-0021_y90_s3401/comparison_summary.json` | `TASK-0009` | Session-level yaw `90` transfer and promotion summary; records `T09-Y1-y90-s3401` as the best strict four-down retained run and `T09-R1-y90-s3401` as the best composite yaw `90` long-run result | retained-batch-summary |
